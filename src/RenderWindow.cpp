...
RenderWindow::RenderWindow()
{
	...
	signTex = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STATIC, SCREEN_WIDTH, SCREEN_HEIGHT + 16);
}
...
void RenderWindow::subscreen(string title)
{
	...
	SDL_Surface* sur = TTF_RenderText_Solid(font, title.c_str(), red);
...
void RenderWindow::updScoreBoard(int score)
{
	TTF_Font* font = TTF_OpenFont("VTC-GarageSale.ttf", 30);
	string scoreInfo = "SCORE: " + to_string(score);
	SDL_Surface* sur = TTF_RenderText_Solid(font, scoreInfo.c_str(), red);
...
{
	TTF_Font* font = TTF_OpenFont("VTC-GarageSale.ttf", 30);
	SDL_Surface* sur = TTF_RenderText_Solid(font, "--NGUYEN PHU HUNG--", blue);
	signTex = SDL_CreateTextureFromSurface(renderer, sur);
	SDL_FreeSurface(sur);
}

void RenderWindow::renderSign()
{
	SDL_Rect dst = { 300, SCREEN_HEIGHT, 200, 16 };
	SDL_RenderCopy(renderer, signTex, NULL, &dst);
}